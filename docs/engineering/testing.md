...
Playwright (`e2e/smoke.spec.ts`, `e2e/revenue-path.spec.ts`, `e2e/payment-path.spec.ts`, `e2e/vendor-fulfillment.spec.ts`, `e2e/refund-path.spec.ts`, `e2e/payout-path.spec.ts`): storefront smokes + authenticated register/login/COD + gateway sandbox redirect + vendor fulfill/ship + customer refund + vendor payout when Nest API + indexed offers (+ `E2E_VENDOR_EMAIL` for vendor-gated journeys) are available (skips otherwise; CI e2e job is frontend-only today). Payment gateway adapters: dual-mode unit specs + mocked credentialed HTTP path; optional sandbox network IT when provider env keys are set (`payment-gateways.sandbox.integration.spec.ts`).
