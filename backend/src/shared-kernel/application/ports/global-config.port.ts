export const GLOBAL_CONFIG_PORT = Symbol('GLOBAL_CONFIG_PORT');
...
export function isAllowedGlobalConfigKey(group: string, key: string): boolean {
  const g = GLOBAL_CONFIG_DEFAULTS[group];
  return Boolean(g && Object.prototype.hasOwnProperty.call(g, key));
}

export function resolveGlobalConfigDefault<T = unknown>(group: string, key: string): T | undefined {
  return GLOBAL_CONFIG_DEFAULTS[group]?.[key] as T | undefined;
}

export interface GlobalConfigPort {
  get<T>(group: string, key: string, defaultValue?: T): Promise<T>;
  set(group: string, key: string, value: unknown): Promise<void>;
  listGrouped(): Promise<Record<string, Record<string, unknown>>>;
  bulkUpdate(
